...
import { getOfflineUpcomingDeliveries } from "../offline/utils/dashboardOffline";
...
const useTailorUpcomingDeliveries = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["tailorUpcomingDeliveries", shopId],
    queryFn: async () => {
      // ── Online path ──────────────────────────────────────────────
      if (navigator.onLine) {
        try {
          const response = await fetch(
            "/api/tailor-dashboard/upcoming-deliveries",
            {
              credentials: "include",
            },
          );
          if (response.ok) {
            ...
            if (shopId) {
              setDashboardCache(shopId, "upcoming-deliveries", json).catch(
                () => {},
              );
            }
            return json;
      ...
      return await getOfflineUpcomingDeliveries(shopId);
    },
    staleTime: 0,
    refetchOnMount: true,
