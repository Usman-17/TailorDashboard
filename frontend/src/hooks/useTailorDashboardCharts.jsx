...
import { getOfflineCharts } from "../offline/utils/dashboardOffline";
...
const useTailorDashboardCharts = () => {
  ...
  useEffect(() => {
    const handleRefresh = () => {
      queryClient.invalidateQueries({ queryKey: ["tailorDashboardCharts"] });
    };
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["tailorDashboardCharts", shopId],
    queryFn: async () => {
      // ── Online path ──────────────────────────────────────────────
      if (navigator.onLine) {
        try {
          ...
          if (response.ok) {
            ...
            if (shopId) {
              setDashboardCache(shopId, "charts", json).catch(() => {});
            }
            return json;
          }
        } catch {
      ...
      return await getOfflineCharts(shopId);
    },
    staleTime: 0,
    refetchOnMount: true,
