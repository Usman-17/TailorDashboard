...
import { getOfflineLatestCustomers } from "../offline/utils/dashboardOffline";
import useGetAuth from "./useGetAuth";

const getEffectiveShopId = (authUser) => {
  if (authUser?.shop?._id) return String(authUser.shop._id);
  if (authUser?.shop && typeof authUser.shop === "string") return String(authUser.shop);
  try {
...
const useTailorLatestCustomers = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["tailorLatestCustomers", shopId],
    queryFn: async () => {
      // ── Online path ──────────────────────────────────────────────
      if (navigator.onLine) {
        try {
          ...
          if (response.ok) {
            ...
            if (shopId) {
              setDashboardCache(shopId, "latest-customers", json).catch(() => {});
            }
            return json;
          }
        } catch {
      ...
      return await getOfflineLatestCustomers(shopId);
    },
    staleTime: 0,
    refetchOnMount: true,
