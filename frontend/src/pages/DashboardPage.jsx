...
const DashboardPage = () => {
  ...
  const statCards = [
    { title: "Total Customers", value: stats?.totalCustomers || 0, icon: Users, color: "bg-sky-50 text-sky-600", trend: "+This month" },
    { title: "Total Orders", value: stats?.totalOrders || 0, icon: Package, color: "bg-emerald-50 text-emerald-600", trend: `${stats?.pendingOrders || 0} pending` },
    { title: "Monthly Revenue", value: `Rs. ${(stats?.monthlyRevenue || 0).toLocaleString()}`, icon: IndianRupee, color: "bg-amber-50 text-amber-600", trend: `Exp: Rs. ${(stats?.monthlyExpenses || 0).toLocaleString()}` },
    { title: "Net Profit", value: `Rs. ${(stats?.netProfit || 0).toLocaleString()}`, icon: Banknote, color: "bg-green-50 text-green-600", trend: "This month" },
    { title: "Today's Deliveries", value: stats?.todayDeliveries || 0, icon: CalendarCheck, color: "bg-purple-50 text-purple-600", trend: `${stats?.overdueOrders || 0} overdue` },
  ];
  ...
  const ordersChartOptions = {
    ...
    markers: { size: 4 },
    xaxis: { categories: charts?.ordersByMonth?.map((o) => o.month) || [], axisBorder: { show: false }, axisTicks: { show: false } },
    yaxis: { min: 0 },
    grid: { borderColor: "#F3F4F6", strokeDashArray: 4 },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        {orderStatusCards.map((card) => (
          <div key={card.title} className="rounded-xl border border-gray-200 bg-white p-4 flex items-center gap-3 hover:shadow-md transition-shadow">
      ...
      {/* Orders Trend */}
      <div className="rounded-xl border border-gray-200 bg-white p-5">
        <h3 className="text-base font-semibold text-gray-800 mb-4">Orders Trend (Last 6 Months)</h3>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <a href="/orders/manage" className="text-sm text-blue-600 hover:text-blue-700 font-medium">View all</a>
          </div>
                ...
                {ordersLoading ? (
                  ...
                  recentOrders.map((order) => (
                    <tr key={order._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-3 text-gray-600">{order.customer?.name || "N/A"}</td>
                      <td className="px-5 py-3 text-gray-600">Rs. {order.totalAmount?.toLocaleString()}</td>
                      <td className="px-5 py-3">
                        <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${STATUS_COLORS[order.status] || "bg-gray-100 text-gray-600"}`}>
        ...
        <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h3 className="text-base font-semibold text-gray-800">Upcoming Deliveries</h3>
            <a href="/orders/manage" className="text-sm text-blue-600 hover:text-blue-700 font-medium">View all</a>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {deliveriesLoading ? (
                  ...
                  <tr><td colSpan="4" className="px-5 py-8 text-center text-gray-400">No upcoming deliveries</td></tr>
                ) : (
                  upcomingDeliveries.map((order) => {
                    ...
                    return (
                      <tr key={order._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        <td className="px-5 py-3 font-medium text-gray-900">{order.orderNumber}</td>
                        <td className="px-5 py-3 text-gray-600">{order.customer?.name || "N/A"}</td>
                        <td className={`px-5 py-3 font-medium ${isOverdue ? "text-red-600" : "text-gray-600"}`}>
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-base font-semibold text-gray-800">Latest Customers</h3>
          <a href="/customer/manage" className="text-sm text-blue-600 hover:text-blue-700 font-medium">View all</a>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-xs uppercase text-gray-500">
            ...
            <tbody>
              {customersLoading ? (
                ...
                <tr><td colSpan="5" className="px-5 py-8 text-center text-gray-400">No customers yet</td></tr>
              ) : (
                latestCustomers.map((customer) => (
                  <tr key={customer._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-3 font-mono text-xs text-gray-500">{customer.customerId}</td>
                    ...
                    <td className="px-5 py-3 text-gray-500">{moment(customer.createdAt).format("DD MMM YYYY")}</td>
                  </tr>
                ))
              )}
            </tbody>
