      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-3">
        {statCards.map((card) => {
          ...
          return (
            <div
              key={card.title}
              className="rounded-2xl border border-gray-200 bg-white p-4 transition-all duration-200 hover:shadow-md flex flex-col justify-between"
            >
              ...
              <div className="mt-3">
                ...
                <p className="text-[11px] text-gray-400 mt-1">{card.subtext}</p>
              </div>
            </div>
          );
        })}
          ...
          <div className="divide-y divide-gray-100 overflow-x-auto mt-3">
            {recentPayments.length === 0 ? (
              ...
              recentPayments.map((pm) => (
                <div
                  key={pm._id}
                  className="py-3 flex items-center justify-between gap-3 text-sm hover:bg-gray-50/50 px-2 rounded-lg transition"
                >
